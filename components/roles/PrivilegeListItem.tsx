...
export function PrivilegeListItem({ privilege, onEdit, onDelete }: PrivilegeListItemProps) {
    return (
        <div className="flex items-center justify-between p-4 bg-white  rounded-2xl border border-gray-100  hover:shadow-md transition-all group">
            <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-gray-900 ">
                    {privilege.display_name || privilege.name}
                </p>
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    <span className="text-[9px] font-mono text-blue-500/70  uppercase tracking-tighter">
                        {privilege.name}
                    </span>
                    {privilege.is_core && (
