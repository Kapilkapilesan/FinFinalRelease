...
export function RolesPrivileges() {
    ...
    return (
        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 h-full">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white  px-10 py-8 rounded-[2.5rem] border border-gray-100  shadow-sm transition-all">
                <div className="space-y-1">
                    <h1 className="text-3xl font-black text-gray-900  tracking-tight flex items-center gap-3">
                        Security Management
                        {loading && <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />}
                    </h1>
                    <p className="text-sm text-gray-500  font-medium opacity-80 uppercase tracking-widest">Configure access roles and system-wide privileges</p>
                </div>
            ...
            <div className="flex flex-col xl:flex-row gap-8 items-start h-full pb-10">
                ...
                <div className={`xl:w-[450px] w-full space-y-8 h-full transition-all duration-500 ease-in-out ${!showSidebar ? 'xl:-ml-[480px] xl:opacity-0 pointer-events-none' : 'opacity-100'}`}>
                    ...
                    <div className="bg-white/70  p-8 rounded-[2.5rem] border border-gray-100  shadow-sm">
                        <div className="flex items-center justify-between mb-8 px-2">
                            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-gray-400">Access Roles</h3>
                            <span className="text-[10px] bg-blue-50  text-blue-600  px-3 py-1 rounded-full font-black">{roles.length} Active</span>
                        </div>
                        <div className="space-y-4 max-h-[350px] overflow-y-auto pr-3 scrollbar-thin scrollbar-thumb-gray-200  scrollbar-track-transparent">
                            {roles.map(role => (
                                <RoleListItem
                                    key={role.id}
                                    role={role}
                    ...
                    <div className="bg-white/50  p-8 rounded-[2.5rem] border border-gray-100  flex flex-col h-full overflow-hidden">
                        <div className="flex items-center justify-between mb-8 px-2 shrink-0">
                            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-gray-400">Capability Dictionary</h3>
                            <span className="text-[10px] bg-green-50  text-green-600  px-3 py-1 rounded-full font-black">{privileges.length} Total</span>
                        </div>

                        <div className="space-y-4 overflow-y-auto pr-3 scrollbar-init scrollbar-thin scrollbar-thumb-gray-200  scrollbar-track-transparent pb-4">
                            {modules.map(moduleName => {
                                ...
                                return (
                                    <div
                                        key={moduleName}
                                        className={`group flex flex-col bg-white  rounded-[2rem] border transition-all duration-300 ${isExpanded
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className={`p-2 rounded-xl transition-colors ${isExpanded ? 'bg-emerald-600 text-white' : 'bg-gray-50  text-gray-400 group-hover:bg-emerald-50  group-hover:text-emerald-500'}`}>
                                                    <Shield size={16} />
                                                </div>
                                                <div>
                                                    <h4 className="text-[11px] font-black text-gray-900  capitalize tracking-tight leading-none">
                                        ...
                                        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
                                            <div className="px-5 pb-5 pt-1 border-t border-gray-50  mt-1">
                                                <div className="space-y-3 overflow-y-auto max-h-[320px] scrollbar-thin scrollbar-thumb-gray-200  pr-2">
                ...
                <div className="flex-1 w-full min-w-0 h-full transition-all duration-500">
                    {selectedRole ? (
                        <div className="bg-white  p-10 rounded-[3rem] border border-gray-100  shadow-xl shadow-gray-200/10  animate-in zoom-in-95 duration-500 h-full overflow-hidden flex flex-col">
                            <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3">
                                                ...
                                                {selectedRole.is_system && (
                                                    <span className="bg-amber-500/10 text-amber-600  text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md border border-amber-500/20">
                                                        System
                                                    </span>
                                                )}
                                            </div>
                                ...
                                <div className="flex items-center gap-3">
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-8 py-4 bg-gray-50  hover:bg-gray-100  rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 border border-gray-100 "
                                    >
                                        <Edit size={16} />
                                        Modify Matrix
                                    </button>
                        ...
                        <div className="h-full min-h-[600px] flex flex-col items-center justify-center bg-gray-50/50  rounded-[3rem] border-2 border-dashed border-gray-200 ">
                            <div className="p-8 bg-white  rounded-[2rem] shadow-2xl mb-8 transform -rotate-3 hover:rotate-0 transition-transform duration-500">
                                <Shield size={64} className="text-blue-500/10" strokeWidth={1} />
                            </div>
                            <h3 className="text-2xl font-black text-gray-900  tracking-tight">Security Matrix Interface</h3>
